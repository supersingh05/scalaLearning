object LearningScala3 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(225); 
  ...
  def upIt(x: String) : String = { return x.toUpperCase};System.out.println("""upIt: (x: String)String""");$skip(71); 
  def upSome() : Unit = {println(upIt("blah")); println(upIt("moo"));};System.out.println("""upSome: ()Unit""");$skip(11); 
  upSome();$skip(113); 
                                                  
  def upIt2(x: String, f: String => String) : String = {f(x)};System.out.println("""upIt2: (x: String, f: String => String)String""");$skip(44); 
  println(upIt2("sdf", x => x.toUpperCase))}
